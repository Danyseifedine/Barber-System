 ...
 *
 * This module handles all service-related operations in the dashboard including:
 ...
 * @function buildApiUrl - Constructs API endpoints for service operations
 *--------------------------------------------------------------------------*/
const defaultErrorHandler = (err) => console.error('Error:', err);
const reloadDataTable = () => serviceTable.reload();
const buildApiUrl = (path) => `${DASHBOARD_URL}/services/${path}`;
 ...
 * Defines the structure and behavior of the Service management table
 *--------------------------------------------------------------------------*/
const tableColumns = [
    {
        "data": "id"
...
const tableColumnDefinitions = [
    { targets: [0], orderable: false, htmlType: 'selectCheckbox' },
    {
        ...
        checkWhen: (data) => data === 'available',
        uncheckWhen: (data) => data === 'unavailable'
    }, {
        targets: [-1],
        htmlType: 'actions',
        className: 'text-end',
...
/*---------------------------------------------------------------------------
 * Bulk Action Handler
 * Processes operations on multiple selected services
 * @param {Array} selectedIds - Array of selected service IDs
 *--------------------------------------------------------------------------*/
const handleBulkActions = (selectedIds) => {
    // Implementation for bulk actions
    // Example: Delete multiple services, change status, etc.
};

/*=============================================================================
 * DataTable Initialization
 * Creates and configures the main service management interface
 *============================================================================*/
export const serviceTable = new $DatatableController('serviceTable', {
