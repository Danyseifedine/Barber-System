/*=============================================================================
 * AppointmentService Management Module
 *
 * This module handles all appointmentService-related operations in the dashboard including:
 ...
 * @function buildApiUrl - Constructs API endpoints for appointmentService operations
 *--------------------------------------------------------------------------*/
const defaultErrorHandler = (err) => console.error('Error:', err);
const reloadDataTable = () => appointmentserviceTable.reload();
const buildApiUrl = (path) => `${DASHBOARD_URL}/appointments/services/${path}`;
...
const userActionHandlers = {
    ...
    edit: function (id) {
        this.callCustomFunction('_EDIT_', id, buildApiUrl(`${id}/edit`), (response) => {
            initSelect2('#appointment_id', "#edit-modal");
            initSelect2('#service_id', "#edit-modal");
        });
    },
 ...
 * Defines the structure and behavior of the AppointmentService management table
 *--------------------------------------------------------------------------*/
const tableColumns = [
    {
        "data": "id"
...
/*---------------------------------------------------------------------------
 * Bulk Action Handler
 * Processes operations on multiple selected appointmentServices
 * @param {Array} selectedIds - Array of selected appointmentService IDs
 *--------------------------------------------------------------------------*/
const handleBulkActions = (selectedIds) => {
    // Implementation for bulk actions
    // Example: Delete multiple appointmentServices, change status, etc.
};

/*=============================================================================
 * DataTable Initialization
 * Creates and configures the main appointmentService management interface
 *============================================================================*/
export const appointmentserviceTable = new $DatatableController('appointmentserviceTable', {
...
// Initialize create appointmentService modal
createModalLoader({
    triggerSelector: '.create',
    endpoint: buildApiUrl('create'),
    onSuccess: (response) => {
        initSelect2('#appointment_id', "#create-modal");
        initSelect2('#service_id', "#create-modal");
    }
});
