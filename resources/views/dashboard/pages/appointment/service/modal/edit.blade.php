<form id="edit-appointmentService-form" form-id="editForm" http-request
    route="{{ route('dashboard.appointments.services.update') }}" identifier="single-form-post-handler" feedback
    close-modal success-toast on-success="RDT">
    <input type="hidden" name="id" id="id" value="{{ $appointmentService->id }}">
        ...
        <select class="form-control form-control-solid" data-control="select2" feedback-id="appointment_id-feedback"
            ...
            @foreach ($appointments as $appointment)
                <option value="{{ $appointment->id }}"
                    {{ $appointmentService->appointment_id == $appointment->id ? 'selected' : '' }}>
        ...
        <select class="form-control form-control-solid" data-control="select2" feedback-id="service_id-feedback"
            ...
            @foreach ($services as $service)
                <option value="{{ $service->id }}"
                    {{ $appointmentService->service_id == $service->id ? 'selected' : '' }}>
